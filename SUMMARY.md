# Summary

* [Rails4でのToDoアプリ開発](firstStep/README.md)
    * [まずは作成されてるアプリを触ってみよう](firstStep/step1.md)
    * [Railsの思想についてちょっとご紹介](firstStep/aboutRails.md)
    * [アプリのひな形を作る便利機能のscaffoldの紹介](firstStep/aboutScaffold.md)
    * [Railsのプロジェクトの構造を理解する](firstStep/aboutRailsProjectStructure.md)    
    * [scaffoldを使ってToDoアプリのひな形を作りましょう](firstStep/createToDoApplication.md)
    * [ToDOアプリのカスタマイズの前に簡単にMVCの概念の紹介](firstStep/aboutMVC.md)
    * [Viewのカスタマイズ](firstStep/aboutView.md)
        * [asssetsの概念の説明](firstStep/aboutAsssets.md)
        * [Rails4でBootstrapを利用する](firstStep/prepareBootstrap.md)
        * [作業前にSublimeTextの便利機能を知る](firstStep/learnEmmetMode.md)
        * [テンプレートファイルの編集をしましょう](firstStep/reviseERB.md)
    * [Controllerのカスタマイズ](firstStep/aboutController.md)
        * [Controllerの役割とルーティングの概念説明](firstStep/aboutControllerAndRouting.md)
        * [デフォルトのルーティングをカスタマイズ](firstStep/reviseRouting.md)
    * [Modelのカスタマイズ](firstStep/aboutModel.md)
        * [Modelの概念とRelationの概念説明](firstStep/aboutModelAndRelation.md)
        * [Modelを気軽に扱うのに便利なPryの説明](firstStep/aboutPry.md)
        * [ユーザー登録機能を作る](firstStep/implementUserManagement1.md)
        * [ユーザー別にToDo情報を管理できる機能を作る](firstStep/implementUserManagement2.md)
* [Google Map API連携アプリを開発する](googlemapsapi/README.md)
* [オリジナル機能を追加してもらう](advanced/README.md)
* [参考情報](reference.md)
