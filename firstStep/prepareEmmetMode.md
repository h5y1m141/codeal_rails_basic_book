...
SublimeTextは標準で主要なプログラミング言語別に文字の色分け（シンタックスハイライト）をしてくれるような最低限の機能が備わっていますが、機能を拡張することが出来ます。

Packageという単位で機能を拡張する事ができるのですが、そのPackageのインストール作業を効率的に行うために、Package Controlというツールをまずはインストールします。
